...
// The confirmation step before a Leave click takes effect.
public final class FellowshipLeaveConfirmScreen {

    private FellowshipLeaveConfirmScreen() {
    }
    ...
    public static void open(ServerPlayerEntity player, Fellowship fellowship, boolean all) {
        ConfirmScreen.open(player, Text.literal("Leave " + fellowship.name() + "?").formatted(Formatting.DARK_GRAY),
    ...
    private static final class Handler extends ConfirmScreen {
        ...
        protected boolean reload() {
            ...
            return fellowship != null && fellowship.isMember(player.getUuid());
        }
        ...
        protected void onConfirm() {
            ...
            members.removeIf(m -> m.id().equals(player.getUuid()));
            store.put(fellowship.withMembers(members));
            store.markDirty();
            ClaimManager.pruneStaleGrants(player.getServer(), fellowship.id());
            player.sendMessage(Text.literal("Left " + fellowship.name()).formatted(Formatting.GRAY), false);
        }
